...
Feature: Unallowed assembly calls are denied.
  ...
  Scenario Outline: Attempt to call an assembly as unauthorized user
    ...
    Then the result is <Result>
      And an audit log entry is stored with user id, ip, event and error, but without sensitive or malformed data,
      And no assembly details are updated
    Examples:
      | Role              | Result                        |
  ...
  Scenario: Attempt to call a future assembly while the pending assembly is ongoing
    Given I am logged in as a role Vorstand or Aufsichtsrat
  ...
  Scenario: Attempt to call an assembly with missing/invalid id
    ...
    Then the result is "error for invalid assembly id"
      And an audit log entry is stored with user id, ip, event and eerror, but without sensitive or malformed data,
      And no assembly details are updated
